...
4. 上層孤立的 `c`：本地規則是孤立 c 畫樹冠、成片 c 才畫霧。若要保留「霧絲」效果，需要遠端交付一張透明的霧 overlay tile（放裝飾層），或作者把地圖的 c 改成成片區域。
5. 拱門正北第 21 列：canopy z_bias 1 會遮住站在該列角色的身體（頭露出）。這是規劃 split 模式的定義結果；若作者覺得太遮，可把 canopy 改回 Y-sort 或縮小樹冠下緣。
6. 船長房 v3 家具色數很高（繩圈 1,159 色、航海圖桌 1,201 個 5-bit 色桶），不是 ART_STYLE_LOCK 的限色像素風；畫面上整組風格一致，本地未退件，但列為風格例外請作者決定是否接受。

## 視覺例外（誠實列出）

- ~~主城上層星空虛空~~：已由填充包解決；孤立 `c` 畫成樹冠（見待確認 4）。
